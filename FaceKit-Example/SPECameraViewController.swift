...
class SPECameraViewController: SPEViewController, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func createAvatar() {
        if let faceImage = self.faceImage {
            FACAvatarManager.currentManager().createAvatarWithGender(gender, faceImage: faceImage, successBlock: { (avatarController) -> Void in
...
protocol SPECameraViewControllerDelegate: NSObjectProtocol {
    optional func cameraViewController(viewController: SPECameraViewController, didCreateAvatarController avatarController: FACAvatarController)
}
